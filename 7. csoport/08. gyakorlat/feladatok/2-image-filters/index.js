...
theFiltersDiv.addEventListener('input', function (e) {
  const checkboxes = [...document.querySelectorAll('#the-filters *:checked')]
  const filterText = checkboxes.map(c => {
    const filter = c.value;
    const range = c.nextElementSibling
    const val = range.value
    return filter.replace('#', val)
  }).join(' ')
  theImage.style.filter = filterText
})


theFiltersDiv.innerHTML = filters.map(f => `
  <label>
